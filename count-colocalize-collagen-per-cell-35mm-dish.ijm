...
function processImage(imageTitle) {
	...
	// AUTO-THRESHOLD apply noise-reducing thresholds to all channels
	threshold(blueImage);
	threshold(greenImage);
	threshold(redImage);

	//Alternatively, consider manual thresholds:
	...
	run("Watershed");
	run("Analyze Particles...", "size=0-Infinity outline display summarize");
